...
# NOTE proxmox RHEL instances dont need powered on 24/7, just for patching - all are currently on beelink
# use proxmox API to power on/off if needed
...
slp=15
echo "o Make sure /etc/ansible/hosts is up to date"
echo "Sleep $slp sec to settle MDNS broadcast after boots..."
~/bin/countdown $slp || sleep $slp
date
...
echo "o NOTE after patching is done, can call  mk-poweroff-rhel.sh  to save resources, 
same set of instances will power down"
...
# srch="rhel|rocky|alma"; (pct list |egrep -i "$srch"; echo '-----'; qm list |egrep -i "$srch") |column -t
